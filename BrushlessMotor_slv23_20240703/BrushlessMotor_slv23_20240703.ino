...
void receiveEvent(int numBytes) {//receive target position
  ...
  if(numBytes >= 2)
  {
    ...
    // Serial.print(", " ); Serial.print(buf[2]);Serial.print(", " ); Serial.println(buf[3]);//Serial.print(", " ); Serial.println(buf[4]);
    targVel = (int16_t)((buf[0] << 8) | buf[1]);//確認必要があるｘｘｘｘｘｘｘｘｘｘｘｘｘｘｘｘｘｘｘｘｘｘｘｘｘ
...
void loop() {
  nowTime = millis();
  // if (i2cReceiveTime < nowTime + 2000) { //   b  \が来ない場合、停止
  //   targVel = 0;
  // }
  ...
  int dir = 0;
  int timeForEver60Degree = 1;

  if (setCurrentVelocity != 0){
    dir = (int)(setCurrentVelocity/abs(setCurrentVelocity));
    timeForEver60Degree = (int)(1000.0 * 60.0 / abs(setCurrentVelocity) / numPole);//to millisecond
  }
  ...
  int k = (int)(500.0 * (nowTime - lastTime) / timeForEver60Degree);
  if (k > 499) k = 499;
  int mD = (int)(1.0*mySin[k] * duty);
  //Serial.print(k); Serial.print(", "); Serial.println(mySin[k]);
  pwm(OUT_U, 3000, mD * U_out[nextPos]);//analogWrite(OUT_U, duty * U_out[nextPos]);
  digitalWrite(SD_U, SUout[nextPos]);
  pwm(OUT_V, 3000, mD * V_out[nextPos]);//analogWrite(OUT_V, duty * V_out[nextPos]);
  digitalWrite(SD_V, SVout[nextPos]);
  pwm(OUT_W, 3000, mD * W_out[nextPos]);//analogWrite(OUT_W, duty * W_out[nextPos]);
